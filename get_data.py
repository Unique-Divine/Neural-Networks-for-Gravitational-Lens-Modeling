...
def get_rotation_corrected(A, B, C):  # this code flips the ellipticity
    # so that (-ex -ey) and (ex, ey) are both evaluated and the best combination
    # is kept (parity invariance)
...
def read_batch_online(X, Y, max_file_num, train_or_test):
    ...
    Xmat = np.transpose(Xmat, axes=(0, 2, 1)).reshape((
            num_samp, numpix_side * numpix_side))
...
def make_real_noise(Fmap):
    ...
    np.conj(np.fliplr(np.flipud(
            Fmap[Npix_2p1 - 1, 1:Npix_2 - 1].reshape((-1, 1))))).shape
    A = np.concatenate((
            Fmap[0:Npix_2, Npix_2p2 - 1:], np.conj(np.fliplr(np.flipud(
            Fmap[Npix_2p1 - 1, 1:Npix_2].reshape((1, -1)))))), axis=0)
    B = np.concatenate((Fmap[0:Npix_2p1, 0:Npix_2p1], A), axis=1)
    C = np.concatenate((np.zeros((Npix_2m1, 1)), np.conj(np.fliplr(np.flipud(
            Fmap[1:Npix_2, Npix_2p2 - 1:]))), np.conj(np.fliplr(np.flipud(
            Fmap[1:Npix_2, 1:Npix_2p1])))), axis=1)
...
def add_gaussian_noise(im):
    if variable_noise_rms == False:
        ...
        rnd_noise_rms = np.random.uniform(low=max_noise_rms / 10,
                                          high=max_noise_rms)
    if np.random.uniform(low=0, high=1) <= 0.25:
        noise_map = np.random.normal(loc=0.0,
                                     scale=rnd_noise_rms, size=im.shape)
    else:
...
def gen_masks(nmax, ARCS, apply_prob=0.5):
    mask = 1.0
    if (np.min(ARCS) < 0.1 and np.max(ARCS) > 0.9 and
            np.random.uniform(low=0, high=1)) <= apply_prob:
        while True:
...
def pick_new_lens_center(ARCS, Y, xy_range=0.5):
    rand_state = np.random.get_state()
    while True:
        x_new = np.random.randint(
                                  -1 * np.ceil(xy_range / 2 / pix_res),
        ...
        shifted_ARCS = im_shift(
                                ARCS.reshape((numpix_side, numpix_side)),
                                m_shift,
                                n_shift
                               ).reshape((numpix_side * numpix_side,))
        if np.sum(shifted_ARCS) >= (0.98 * np.sum(ARCS)):
            break

        # lensXY = np.array( [ np.double(x_new) * pix_res+ (Y[3]%pix_res) ,
        # np.double(y_new) * pix_res + (Y[4]%pix_res) ])
    lensXY = np.array([
                       np.double(m_shift) * pix_res + Y[3],
                       np.double(n_shift) * pix_res + Y[4]
                      ])
    np.random.set_state(rand_state)
...
def read_data_batch(X, Y, mag, max_file_num, train_or_test):
    ...
    for i in range(batch_size):
        ...
        if nt > 1:
            ...
            arc_filename = d_path[pick_folder] + train_or_test + '_' + \
                           "%07d" % (inds[i] + 1) + '.png'
            lens_filename = d_lens_path[pick_folder] + train_or_test + '_' + \
                            "%07d" % (inds[i] + 1) + '.png'
            if train_or_test == 'test':
        ...
        ARCS_SHIFTED, lensXY, m_shift, n_shift = (
            pick_new_lens_center(ARCS, Y[i, :], xy_range=max_xy_range))
        LENS_SHIFTED = (
                im_shift(
                        LENS.reshape((numpix_side, numpix_side)),
                        m_shift,
                        n_shift
                ).reshape((numpix_side * numpix_side,)))
        ...
        if ((np.all(np.isnan(ARCS) == False))
                and ((np.all(ARCS >= 0))
                        and (np.all(np.isnan(Y[i, 3:5]) == False)))
                and ~np.all(ARCS == 0)):
            break
